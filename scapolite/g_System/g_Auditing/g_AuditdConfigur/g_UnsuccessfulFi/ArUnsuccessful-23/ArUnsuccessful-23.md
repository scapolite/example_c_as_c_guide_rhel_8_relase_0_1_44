...
scapolite:
    class: rule
    version: '0.51'
id: audit_rules_unsuccessful_file_modification_openat_o_trunc_write
id_namespace: org.ssgproject.RHEL-8
title: Record Unauthorized Modification Attempts to Files - openat O\_TRUNC
rule: <see below>
rationale: <see below>
  ...
  - relative_id: '01'
    ...
    automations:
      - system: urn:xccdf:fix:script:sh
        href: audit_rules_unsuccessful_file_modification_openat_o_trunc_write.sh
checks:
  - relative_id: '01'
    automations:
      - system: http://oval.mitre.org/XMLSchema/oval-definitions-5
        idref: oval:ssg-audit_rules_unsuccessful_file_modification_openat_o_trunc_write:def:1
        href: audit_rules_unsuccessful_file_modification_openat_o_trunc_write.oval.xml
  - relative_id: '02'
    automations:
      - system: http://scap.nist.gov/schema/ocil/2
        idref: ocil:ssg-audit_rules_unsuccessful_file_modification_openat_o_trunc_write_ocil:questionnaire:1
        href: audit_rules_unsuccessful_file_modification_openat_o_trunc_write.ocil.xml
crossrefs:
...
users and root. The `openat` syscall can be used to modify files if
called for write operation of with O\_TRUNC flag. The following auidt
rules will asure that unsuccessful attempts to modify a file via
`openat` syscall are collected. If the `auditd` daemon is configured to
use the `augenrules` program to read audit rules during daemon startup
(the default), add the rules below to a file with suffix `.rules` in the
