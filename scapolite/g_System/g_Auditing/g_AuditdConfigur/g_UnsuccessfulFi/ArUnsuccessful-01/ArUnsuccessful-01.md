...
scapolite:
    class: rule
    version: '0.51'
id: audit_rules_unsuccessful_file_modification_openat_rule_order
id_namespace: org.ssgproject.RHEL-8
title: |
    Ensure auditd Rules For Unauthorized Attempts To openat Are Ordered
    Correctly
rule: <see below>
rationale: <see below>
  ...
  - relative_id: '01'
    ...
    automations:
      - system: urn:xccdf:fix:script:sh
        href: audit_rules_unsuccessful_file_modification_openat_rule_order.sh
checks:
  - relative_id: '01'
    automations:
      - system: http://oval.mitre.org/XMLSchema/oval-definitions-5
        idref: oval:ssg-audit_rules_unsuccessful_file_modification_openat_rule_order:def:1
        href: audit_rules_unsuccessful_file_modification_openat_rule_order.oval.xml
  - relative_id: '02'
    automations:
      - system: http://scap.nist.gov/schema/ocil/2
        idref: ocil:ssg-audit_rules_unsuccessful_file_modification_openat_rule_order_ocil:questionnaire:1
        href: audit_rules_unsuccessful_file_modification_openat_rule_order.ocil.xml
crossrefs:
...
unsuccessful modification and unsuccessful access of files via `openat`
syscall the audit rules collecting these events need to be in certain
order. The more specific rules need to come before the less specific
rules. The reason for that is that more specific rules cover a subset of
events covered in the less specific rules, thus, they need to come
before to not be overshadowed by less specific rules, which match a
bigger set of events. Make sure that rules for unsuccessful calls of
`openat` syscall are in the order shown below. If the `auditd` daemon is
configured to use the `augenrules` program to read audit rules during
daemon startup (the default), check the order of rules below in a file
with suffix `.rules` in the directory `/etc/audit/rules.d`. If the
`auditd` daemon is configured to use the `auditctl` utility to read
audit rules during daemon startup, check the order of rules below in
