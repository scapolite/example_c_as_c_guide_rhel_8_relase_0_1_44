  ...
  - relative_id: '01'
    ...
    automations:
      - system: urn:xccdf:fix:script:sh
        href: audit_rules_session_events.sh
checks:
  - relative_id: '01'
    automations:
      - system: http://oval.mitre.org/XMLSchema/oval-definitions-5
        idref: oval:ssg-audit_rules_session_events:def:1
        href: audit_rules_session_events.oval.xml
crossrefs:
...
The
audit system already collects process information for all users and
root. If the `auditd` daemon is configured to use the `augenrules`
program to read audit rules during daemon startup (the default), add the
following lines to a file with suffix `.rules` in the directory
`/etc/audit/rules.d` in order to watch for attempted manual edits of
files involved in storing such process information:
