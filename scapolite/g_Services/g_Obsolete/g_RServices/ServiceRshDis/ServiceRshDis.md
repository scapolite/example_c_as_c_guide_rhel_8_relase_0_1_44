  ...
  - relative_id: '01'
    automations:
      - system: http://scap.nist.gov/schema/ocil/2
        idref: ocil:ssg-service_rsh_disabled_ocil:questionnaire:1
        href: service_rsh_disabled.ocil.xml
crossrefs:
...
The
rsh service uses unencrypted network communications, which means that
...
as a service through xinetd or separately as a systemd socket, should be
disabled. If using xinetd, set `disable` to `yes` in
`/etc/xinetd.d/rsh`. The `rsh` socket can be disabled with the following
command:
